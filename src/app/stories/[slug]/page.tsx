...
const storyData: Record<string, Story> = {
    "the-symphony-of-hanoi-dawn": {
        ...
        image: "/Pho.jpg",
        content: `Khi những tia nắng đầu tiên chưa kịp chạm tới những mái ngói rêu phong của phố cổ Hà Nội, mùi hương nồng nàn của quế và hồi đã len lỏi khắp các ngõ nhỏ. Đó là lúc những nồi nước dùng Phở bắt đầu "thì thầm".
...
export default function StoryDetailPage({ params }: { params: { slug: string } }) {
    const story = storyData[params.slug] ?? storyData["the-symphony-of-hanoi-dawn"]
        ...
        <main className="bg-[#F9F6F2] min-h-screen pb-24">
            ...
            <div className="relative h-[70vh] w-full">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center text-white px-8">
                    <p className="uppercase tracking-[0.3em] font-bold text-sm mb-4 text-orange-400">
                        {story.dish}
                    </p>
                    <h1 className="text-6xl md:text-8xl font-serif font-bold text-center max-w-4xl">
                        {story.title}
                    </h1>
                </div>
                <Link
                    href="/stories"
                    className="absolute top-10 left-10 bg-white/20 backdrop-blur-md px-6 py-2 rounded-full text-white font-bold border border-white/20 hover:bg-white/40 transition-all"
                >
                    ← Back to Stories
                </Link>
            </div>
            ...
            <article className="max-w-3xl mx-auto px-8 -mt-20 relative z-10">
                <div className="bg-white p-12 md:p-20 rounded-[40px] shadow-2xl shadow-orange-900/10">
                    <div className="flex items-center gap-4 mb-10 pb-10 border-b border-orange-100">
                        ...
                        <div>
                            <p className="font-bold text-[#2D1B0D]">
                                {story.author ?? 'FlavorNest'}
                            </p>
                    ...
                    <div className="mt-16 pt-10 border-t border-orange-100 text-center">
                        <p className="italic text-gray-400 mb-6">
                            "Cooking is the art of storytelling through taste."
                        </p>
