...
export const metadata = {
    title: 'Northern Vietnamese Cuisine | Ẩm Thực Miền Bắc | FlavorNest Vietnam',
    description: 'Discover the soul of Northern Vietnamese cooking — from Hanoi\'s legendary Phở Bò to Bún Chả and Bánh Cuốn. Authentic recipes rooted in centuries of culinary tradition.',
    keywords: 'northern vietnamese food, hanoi recipes, pho bo, bun cha, banh cuon, am thuc mien bac',
    openGraph: {
        title: 'Northern Vietnamese Cuisine | FlavorNest Vietnam',
        description: 'Recipes rooted in centuries of Hanoi culinary tradition.',
        images: ['https://images.unsplash.com/photo-1569050467447-ce54b3bbc37d?w=1200&q=80'],
    },
}
...
export default function NorthernCuisinePage() {
    ...
    return (
        <main style={{ minHeight: '100vh', background: '#0D0A07', fontFamily: "'DM Sans', system-ui, sans-serif", color: 'white' }}>
            ...
            {/* ── EXPLORE MORE ── */}
            <section style={{ borderTop: '1px solid rgba(255,255,255,0.06)', padding: '64px 32px 88px' }}>
                <div style={{ maxWidth: 1200, margin: '0 auto', textAlign: 'center' }}>
                    ...
                    <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: 34, fontWeight: 700, color: 'white', margin: '0 0 36px' }}>
                    ...
                    <div style={{ display: 'flex', gap: 14, justifyContent: 'center', flexWrap: 'wrap' }}>
                        {[
                            ...
                            <Link key={item.href} href={item.href} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, padding: '20px 28px', borderRadius: 20, border: '1px solid rgba(255,255,255,0.09)', background: 'rgba(255,255,255,0.03)', textDecoration: 'none', transition: 'all 0.22s', minWidth: 150 }}
                                onMouseEnter={e => { const el = e.currentTarget as HTMLAnchorElement; el.style.borderColor = 'rgba(217,119,6,0.4)'; el.style.background = 'rgba(217,119,6,0.06)'; el.style.transform = 'translateY(-4px)'; }}
                                onMouseLeave={e => { const el = e.currentTarget as HTMLAnchorElement; el.style.borderColor = 'rgba(255,255,255,0.09)'; el.style.background = 'rgba(255,255,255,0.03)'; el.style.transform = 'none'; }}
                            >
                                <span style={{ fontSize: 28 }}>{item.emoji}</span>
                                <span style={{ fontSize: 14, fontWeight: 600, color: 'white' }}>{item.label}</span>
                                <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.35)', fontStyle: 'italic' }}>{item.sub}</span>
                            </Link>
                        ))}
                    </div>
                </div>
