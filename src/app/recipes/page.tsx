    ...
    const filtered = useMemo(() => {
        ...
        return ALL_CONTENT.filter((r) => {
            ...
            const matchCat = (cat: string) => {
                ...
                if (cat === "Rice") {
                    if (/\b(noodle|pho|bun|bún|ph?|mi quang|vermicelli|tapioca)\b/.test(titleLower)) return false;
                    if (/\b(congee|chao|cháo)\b/.test(titleLower)) return false;
                    return /\b(rice|com|com|xoi|xôi)\b/.test(titleLower) || tagsLower.includes("rice");
                }

                if (cat === "Noodles") {
                    if (/\b(rice|com|com|xoi|xôi)\b/.test(titleLower) && !/\b(noodle|pho|bun|bún|ph?|mì)\b/.test(titleLower)) return false;
                    return /\b(noodle|pho|bun|bún|ph?|mì|mi quang|vermicelli|tapioca)\b/.test(titleLower) || tagsLower.includes("noodles");
                }

                if (cat === "Soup") {
                    return /\b(soup|canh|broth|pho|ph?|bun bo|bún bò|bun rieu|bún riêu|congee|chao|cháo)\b/.test(titleLower);
                }
