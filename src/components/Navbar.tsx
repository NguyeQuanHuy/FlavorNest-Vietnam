        ...
        cols: [
            {
                head: "Categories",
                items: [
                    { name: "Breakfast", url: "/recipes/breakfast" },
                    { name: "Main Dishes", url: "/recipes/main-dishes" },
                    { name: "Desserts", url: "/recipes/desserts" },
                    { name: "Specialty Drinks", url: "/recipes/drinks" },
                ]
            },
            {
                head: "Regions",
                items: [
                    { name: "Northern Cuisine", url: "/recipes/north" },
                    { name: "Central Cuisine", url: "/recipes/central" },
                    { name: "Southern Cuisine", url: "/recipes/south" },
                    { name: "Street Food", url: "/recipes/street-food" },
                ]
            }
        ]
    };
            ...
            <div className="max-w-7xl mx-auto px-8 h-20 flex items-center justify-between">
                ...
                <div className="hidden md:flex items-center gap-10 h-full">
                    ...
                    <div className="group relative h-full flex items-center">
                        ...
                        <div className="absolute top-full left-1/2 -translate-x-1/2 pt-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-50">
                            <div className="bg-white rounded-2xl shadow-2xl border border-fn-brown/5 p-6 w-[440px] grid grid-cols-2 gap-8">
                                {recipesMenu.cols.map((col, idx) => (
                                    <div key={idx}>
                                        <p className="text-[10px] font-black text-fn-amber uppercase tracking-[0.2em] mb-4 border-b border-fn-amber/10 pb-1">{col.head}</p>
                                        <div className="flex flex-col gap-3">
                                            {col.items.map((item, i) => (
                                                <Link key={i} href={item.url} className="text-sm text-fn-brown/70 hover:text-fn-amber transition-all">{item.name}</Link>
                                            ))}
                                        </div>
                                    </div>
                                ))}
                                <div className="col-span-2 pt-4 border-t border-fn-brown/5 text-center">
                                    <Link href={recipesMenu.link} className="text-[10px] font-bold text-fn-brown/40 hover:text-fn-amber uppercase tracking-[0.1em]">{recipesMenu.title} →</Link>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="group relative h-full flex items-center">
                        ...
                        <div className="absolute top-full left-1/2 -translate-x-1/2 pt-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-50">
                            <div className="bg-white rounded-2xl shadow-2xl border border-fn-brown/5 p-6 w-[440px] grid grid-cols-2 gap-8">
                                {storiesMenu.cols.map((col, idx) => (
                                    <div key={idx}>
                                        <p className="text-[10px] font-black text-fn-amber uppercase tracking-[0.2em] mb-4 border-b border-fn-amber/10 pb-1">{col.head}</p>
                                        <div className="flex flex-col gap-3.5">
                                            {col.items.map((item, i) => (
                                                <Link key={i} href={item.url} className="flex flex-col group/item">
                                                    <span className="text-sm font-semibold text-fn-brown/80 group-hover/item:text-fn-amber transition-colors">{item.name}</span>
                                                    {item.desc && <span className="text-[10px] text-fn-brown/40 -mt-0.5">{item.desc}</span>}
                                                </Link>
                                            ))}
                                        </div>
                                    </div>
                                ...
                                <div className="col-span-2 pt-4 border-t border-fn-brown/5 text-center">
                                    <Link href={storiesMenu.link} className="text-[10px] font-bold text-fn-brown/40 hover:text-fn-amber uppercase tracking-[0.1em]">{storiesMenu.title} →</Link>
                                </div>
                            </div>
                        </div>
                        <span className="absolute bottom-5 left-0 w-0 h-0.5 bg-fn-amber transition-all group-hover:w-full"></span>
                ...
                <div className="flex items-center gap-4 sm:gap-5">
                    ...
                    <div className="h-8 w-[1px] bg-fn-brown/10 hidden sm:block"></div>
